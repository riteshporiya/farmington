...
var tableName = $('#inquiresTable').DataTable({
  scrollX: true,
  deferRender: true,
  scroller: true,
  processing: true,
  ...
  columns: [{
    ...
    data: function data(row) {
      ...
      return prepareTemplateRender('#inquiresTemplate', data);
    },
    name: 'id'
  }]
});
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        ...
        isEmpty(result.data.message) ? $('#showMessage').text('N/A') : $('#showMessage').text(result.data.message);
